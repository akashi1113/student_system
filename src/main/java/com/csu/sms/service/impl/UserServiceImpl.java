...
public class UserServiceImpl implements UserService {
    ...
    public Map<String, Object> login(String username, String password) {
        ...
        // 密码验证：先尝试BCrypt验证，失败则尝试明文比较
        boolean passwordValid = false;
        try {
            passwordValid = passwordEncoder.matches(password, user.getPassword());
        } catch (Exception e) {
            log.warn("BCrypt password verification failed for user {}, trying plain text comparison", username);
        }
        
        // 如果BCrypt验证失败，尝试明文比较（兼容旧数据）
        if (!passwordValid) {
            passwordValid = password.equals(user.getPassword());
            if (passwordValid) {
                log.info("User {} logged in with plain text password, consider updating to encrypted password", username);
            }
        }
